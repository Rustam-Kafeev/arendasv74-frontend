    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {existingPhotos.map((photo, idx) => {
                ...
                return (
                  <div key={idx} className="relative w-20 h-20 border rounded overflow-hidden group">
                    ...
                    <button type="button" onClick={() => removeExistingPhoto(idx)} className="absolute top-0 right-0 bg-red-500 text-white p-1 rounded-bl hover:bg-red-600">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                );
        ...
        <div>
          ...
          <input type="file" multiple accept="image/*" onChange={handleNewFiles} ref={fileInputRef} className="hidden" id="new-photos" />
          <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-gray-300 rounded-lg p-4 flex flex-col items-center cursor-pointer hover:bg-gray-50">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {previewUrls.map((url, idx) => (
                <div key={idx} className="relative w-20 h-20 border rounded overflow-hidden group">
                  ...
                  <button type="button" onClick={() => removeNewPhoto(idx)} className="absolute top-0 right-0 bg-red-500 text-white p-1 rounded-bl hover:bg-red-600">
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              ))}
